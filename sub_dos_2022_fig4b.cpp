...
g++ -I eigen/ sub_dos_2022_fig4b.cpp -o program3
...
when calculating  sum of hp across tissues, 
first calculate [a]free and [b]free in balance and out of balance tissues
and
sum of [hp] = affected_tissue*hp_per_tissue_in_balance + unaffected_tissue*hp_per_tissue_out_of_balance;
and
...
// Figure parameters    
    ...
    std::string trial_number = "zoomed_out";    
...
// Name and Initiate .csv file to print to    
    std::ofstream myfile;
    std::string file_name = "fig" + figure_number + "_combined_vers4_ss" + trial_number +".csv";
    myfile.open (file_name);
// Label column headings    
    myfile << "Prob Fix,    Scalar, Ne, Time,   ";
...
//                    std::cout << "fitness: " << fitness_per_state << "\n\n";
            ...
            //------Print to .csv file--------------------------------------------                  
                myfile << Prob_fixation << ", "  << scalar << ",   "  << N << ",  "  << time;
...
//Sum hp across all regulatory domains
    ...
    long double sum_of_hp_across_tissues_for_state = affected_tissue*hp_per_tissue_in_balance + unaffected_tissue*hp_per_tissue_out_of_balance;
...
long double which_prob_fix_calculation(long double fitness_current_state, long double fitness_next_state, long double N, int prob_of_fix_which){
// Decide which prob of fixation calculation to use (sella and hirsch or 1/N) and calculate it
    long double this_prob_fix;
    if(prob_of_fix_which == 0){
//------Probability of fixation proportional to hp---------------------  
        this_prob_fix = calculate_prob_fix_using_sella_hirsch(fitness_current_state, fitness_next_state, N);
    }
    else if(prob_of_fix_which == 1){
//------Probability of fixation = 1/N-----------------------------------
        this_prob_fix = calculate_prob_fix_as_one_over_Ne(N); 
    }
    else{
        std::cout << "ERROR";
        this_prob_fix = 0.00; 
